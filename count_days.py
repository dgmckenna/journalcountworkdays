...
header_non_tags = ['Date', 'Duration'] 

# For each item where we found the regex of the form {YYYY-MM-DD, duration, tag1, tag2}
for item in regex_days_list:
    ...
    for tag in item_list_tmp[len(header_non_tags):]:
        s_tag = tag.strip()
...
for item in regex_days_list:
    ...
    for i in range(len(header_non_tags)):
        wide_row[i] = item_list_tmp[i]
